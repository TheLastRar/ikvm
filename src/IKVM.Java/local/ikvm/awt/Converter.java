...
import cli.System.Drawing.Drawing2D.FillMode;
import cli.System.Drawing.Drawing2D.GraphicsPath;
import cli.System.Drawing.Drawing2D.LineJoin;
...
class J2C {
    ...
    static Color ConvertColor(java.awt.Color color) {
        return color == null ? Color.Empty : Color.FromArgb(color.getRGB());
    }

    static Bitmap ConvertImage(java.awt.Image img) {
        if (img instanceof BufferedImage) {
            return ((BufferedImage) img).getBitmap();
        }
        // if (img instanceof NetVolatileImage)
        // {
        // return ((NetVolatileImage)img).bitmap;
        // }
        // if (img instanceof sun.awt.image.ToolkitImage)
        // {
        // sun.awt.image.ImageRepresentation ir =
        // ((sun.awt.image.ToolkitImage)img).getImageRep();
        ...
        // }
        // if (img instanceof NoImage)
        // {
        // return null;
        // }
        try {
            throw new UnsupportedOperationException("Image class:" + img.getClass().getName());
    ...
    static PointF ConvertPoint(java.awt.geom.Point2D point) {
        return new PointF((float) point.getX(), (float) point.getY());
    }

    static RectangleF ConvertRect(java.awt.geom.Rectangle2D rect) {
        return new RectangleF((float) rect.getX(), (float) rect.getY(), (float) rect.getWidth(),
                (float) rect.getHeight());
    }

    static Rectangle ConvertRect(java.awt.Rectangle rect) {
    ...
    /// <param name="arcWidth">the horizontal diameter of the arc at the four
    /// corners</param>
    /// <param name="arcHeight">the vertical diameter of the arc at the four
    /// corners</param>
    /// <returns></returns>
    static GraphicsPath ConvertRoundRect(int x, int y, int w, int h, int arcWidth, int arcHeight) {
        GraphicsPath gp = new GraphicsPath();
        boolean drawArc = arcWidth > 0 && arcHeight > 0;
        int a = arcWidth / 2;
        int b = arcHeight / 2;
    ...
    static GraphicsPath ConvertShape(java.awt.Shape shape) {
        java.awt.geom.PathIterator iterator = shape.getPathIterator(null);
        GraphicsPath gp = new GraphicsPath();
        gp.set_FillMode(FillMode.wrap(iterator.getWindingRule()));
    ...
    static LineJoin ConvertLineJoin(int join) {
        switch (join) {
            case java.awt.BasicStroke.JOIN_MITER:
                return LineJoin.wrap(LineJoin.Miter);
            case java.awt.BasicStroke.JOIN_ROUND:
                return LineJoin.wrap(LineJoin.Round);
            case java.awt.BasicStroke.JOIN_BEVEL:
                return LineJoin.wrap(LineJoin.Bevel);
            default:
                System.out.println("Unknown line join type:" + join);
                return LineJoin.wrap(LineJoin.Miter);
        }
    }

    static Matrix ConvertTransform(java.awt.geom.AffineTransform tx) {
        return new Matrix(
                (float) tx.getScaleX(),
                (float) tx.getShearY(),
                (float) tx.getShearX(),
    ...
    // static Font ConvertFont(String name, int style, float size)
    // {
    // if (size <= 0)
    // {
    // size = 1;
    ...
    // {
    // //Some Fonts (for example Aharoni) does not support Regular style. This throw
    // an exception else it is not documented.
    ...
    static Region ConvertRegion(sun.java2d.pipe.Region shape) {
        if (shape == null) {
            return null;
        }
    ...
    static String ConvertGlyphVector(java.awt.font.GlyphVector gv) {
...
/// </summary>
class C2J {
    static java.awt.geom.AffineTransform ConvertMatrix(Matrix matrix) {
        float[] elements = matrix.get_Elements();
        return new java.awt.geom.AffineTransform(elements);
    }

    static java.awt.Rectangle ConvertRectangle(RectangleF rec) {
        return new java.awt.Rectangle((int) rec.get_X(), (int) rec.get_Y(), (int) rec.get_Width(),
                (int) rec.get_Height());
    }
    ...
    // static java.awt.Font ConvertFont(Font font)
    // {
    // float size = font.Size;
    // if (font.Unit != GraphicsUnit.Pixel)
    // {
    // size = font.SizeInPoints *
    // java.awt.Toolkit.getDefaultToolkit().getScreenResolution() / 72;
    // }
    // java.awt.Font jFont = new java.awt.Font(font.Name, (int)font.Style,
    // (int)size);
    // if (jFont.getSize2D() != size)
    // {
    // jFont = jFont.deriveFont(size);
    // }
    // //TODO performance we should set the .NET Font, we can do it with an
    // aditional constructor.
    // return jFont;
    // }

    static java.awt.Shape ConvertShape(GraphicsPath path) {
        java.awt.geom.GeneralPath shape = new java.awt.geom.GeneralPath();
        shape.setWindingRule(path.get_FillMode().Value);

        int pointCount = path.get_PointCount();
        if (pointCount > 0) {
            // get these here because a lot of processing goes on to generate these arrays
            PointF[] points = path.get_PathPoints();
            byte[] types = path.get_PathTypes();
            ...
            for (int i = 0; i < pointCount; i++) {
                ...
                switch (type) {
                    case 0:
                        ...
                        shape.moveTo(point.get_X(), point.get_Y());
                        break;
                    case 1:
                        // Indicates that the point is one of the two endpoints of a line.
                        shape.lineTo(point.get_X(), point.get_Y());
                        break;
                    case 3:
                        // Indicates that the point is an endpoint or control point of a cubic B?zier
                        // spline.
                        PointF point2 = points[++i];
                        PointF point3 = points[++i];
                        shape.curveTo(point.get_X(), point.get_Y(), point2.get_X(), point2.get_Y(), point3.get_X(),
                                point3.get_Y());
                        pathType = types[i];
                        break;
                    default:
                        System.out.println("Unknown GraphicsPath type: " + type);
                        break;
                }
                if ((pathType & 0x80) > 0) {
